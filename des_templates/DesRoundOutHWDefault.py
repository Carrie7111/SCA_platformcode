class DesRoundOutHWDefault:
    ...
    def permute(self, in_val, mat, insize=None):
        if insize is None:
            insize = len(mat)
        result = 0
        i = 0
    ...
    def invPermute(self, in_val, mat, outsize=None):
        if outsize is None:
            outsize = len(mat)
        result = 0
        i = 0
    ...
    def sbox(self, in_val):
        result = 0
        for i in range(8):
            result = ((result << 4) + self.S[i][(in_val >> (6 * (7 - i))) & 0x3F])
        return result

    def bytetolong(self, ba, offset, length):
        result = 0
    ...
    def GetMidDataHW(self, data):
        ...
        for i in range(64):
            ...
            s_out ^= self.invPermute(Des_L, self.P)
            index_skey = i
            index_sbit = 28
