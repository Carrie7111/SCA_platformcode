...
class DesSboxIn:
    ...
    def reportKeyRetrieval(self, key, best):
        if key == 0:
            self.newRoundKey = 0
        roundedValue = best[0][2] * self.precision
        roundedValue = math.floor(roundedValue + 0.5) / self.precision
        if roundedValue < 0:
            self.newRoundKey = (self.newRoundKey << 6) + int(best[1][0])
        else:
    ...
    def guessNextRoundKey(self, best):
        ...
        if self.round == 0 or self.round == 3:
            ...
            boxes = self.hw(mask)
            if boxes > 0:
                print(f"{self.mismatches} 第{boxes}个S盒子对应的子密钥不匹配")
            step = 1
            tests = 0
            while True:
    ...
    def makeKey(self, best, index, mask, step):
        ...
        for i in range(self.keys):
            ...
            if ((mask >> k_idx) & 1) == 1:
                j = index % step
                index //= step
            roundedValue = best[i][j][2] * self.precision
            roundedValue = math.floor(roundedValue + 0.5) / self.precision
            if roundedValue < 0:
                key = (key << self.keyBits) + int(best[i][(j + 1) % step][0])
            else:
