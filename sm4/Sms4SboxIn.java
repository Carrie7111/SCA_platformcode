...
@ModuleDesc(pkgName="SM4", name="SM4 S盒输入攻击分析", desc="", isVisible=true)
public class Sms4SboxIn extends DifferentialAnalysis implements ActionListener {
    ...
    public float[] select(Trace t) {
        ...
        for (int guess = 0; guess < 256; guess++) {
            long value = Sms4.u32(sIn ^ BYTE_GUESS[guess]);
    ...
    public void report(int key, Vector<float[]> best, int fragmentOffset, int fragmentEnd) {
        ...
        newRoundKey = Sms4.u32((newRoundKey << 8) | chooseByte(best));
