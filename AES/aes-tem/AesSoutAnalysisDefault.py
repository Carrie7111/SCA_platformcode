# 模拟父类 AesSoutAnalysisBase（你的项目已有则删除）
class AesSoutAnalysisBase:
    pass


# 标准 AES S 盒（与 Java 中 Aes.S 完全一致，必须保留）
class Aes:
    S = [
        
    ]


# 核心实现类：完全对齐 Java AesSoutAnalysisDefault
class AesSoutAnalysisDefault(AesSoutAnalysisBase):
    """
    AES S盒输出攻击分析
    功能、运算、返回值 与原Java代码完全一致
    """

    def GetMidDataHW(self, data):
        # 空值判断（和 Java 完全一致）
        if data is None:
            return None

        # 初始化 4096 长度 float 数组（对应 Java float[]）
        MidDataHW = [0.0] * 4096
        # 初始化 16 字节临时数组
        tem = bytearray(16)

        # 外层循环：猜测密钥 i 0~255
        for i in range(256):
            # 第一步：data[j] ^ i 赋值到 tem
            for j in range(16):
                tem[j] = data[j] ^ i

            # 第二步：计算 S 盒输出的汉明重量
            for j in range(16):
                s_in = tem[j]
                # 读取 S 盒值 + 无符号处理
                s_out = Aes.S[s_in & 0xFF]
                # 数组索引：i + 256 * j（完全不变）
                index = i + 256 * j
                # 计算汉明重量并转为 float
                MidDataHW[index] = float(self.GetHW(s_out & 0xFF))

        return MidDataHW

    # 汉明重量计算（来自父类，二进制 1 的个数）
    def GetHW(self, value):
        return bin(value).count('1')