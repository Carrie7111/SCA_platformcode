...
class DesSboxInOutHD:
    ...
    def updateEnabledPanels(self):
        if self.round1Button.isSelected():
    ...
    def _setDialogValuesInternal(self):
        self.setInt(self.trackPanel, "bit")
        self.setInt(self.roundPanel, "round")
        self.setBoolean(self.tdesCheckBox, "tdes")
        self.setInt(self.encryptPanel, "encrypt")
        self.setInt(self.targetPanel, "target")
        self.setInt(self.modelPanel, "model")
        self.setInt(self.whichDesPanel, "which.des")
        self.setInt(self.keySizePanel, "key.size")
        self.roundKey = self.getLong("round.key")
        if self.roundKey != 0:
            self.roundKeyTextField.config(text=self.formatNumberString(format(self.roundKey, 'x').upper(), 2))
        else:
            self.roundKeyTextField.config(text="")
        self.desKey = self.getLong("des.1.key")
        if self.desKey != 0:
            self.desKeyTextField.config(text=self.formatNumberString(format(self.desKey, 'x').upper(), 2))
        else:
            self.desKeyTextField.config(text="")
        self.des2Key = self.getLong("des.2.key")
        if self.des2Key != 0:
            self.des2KeyTextField.config(text=self.formatNumberString(format(self.des2Key, 'x').upper(), 2))
        else:
            self.des2KeyTextField.config(text="")
        self.des3Key = self.getLong("des.3.key")
        if self.des3Key != 0:
            self.des3KeyTextField.config(text=self.formatNumberString(format(self.des3Key, 'x').upper(), 2))
        else:
            self.des3KeyTextField.config(text="")
        self.updateEnabledPanels()

    def setInt(self, panel, key):
        if key == "bit":
            self.bit = self.get(key)
        elif key == "round":
            self.selectedRound = self.get(key)
        elif key == "encrypt":
            self.encryptOption = self.get(key)
        elif key == "target":
            self.target = self.get(key)
        elif key == "model":
            self.model = self.get(key)
        elif key == "which.des":
            self.targetDes = self.get(key)
        elif key == "key.size":
            self.keySize = self.get(key)

    def setBoolean(self, checkbox, key):
        self.tdes = self.get(key)
    ...
    def getDialogValues(self):
        self.getKeyValues()
        self.targetDes = self.getIntFromPanel(self.whichDesPanel, "which.des")
        self.bit = self.getIntFromPanel(self.trackPanel, "bit")
        self.selectedRound = self.getIntFromPanel(self.roundPanel, "round")
        self.tdes = self.getBooleanFromPanel(self.tdesCheckBox, "tdes")
        self.encrypt = (self.encryptOption == self.DES_ENCRYPT)
        self.encryptOption = self.getIntFromPanel(self.encryptPanel, "encrypt")
        if not self.tdes:
            self.targetDes = 0
        self.round = self.selectedRound
        self.target = self.getIntFromPanel(self.targetPanel, "target")
        self.model = self.getIntFromPanel(self.modelPanel, "model")
        self.target = 0
        self.model = 1
        self.keySize = self.getIntFromPanel(self.keySizePanel, "key.size")
        self.dataLength = 512
        self.targets = 1
    ...
    def getIntFromPanel(self, panel, key):
        if key == "bit":
            return self.bitVar.get()
        elif key == "round":
            return self.roundVar.get()
        elif key == "encrypt":
            return self.encryptVar.get()
        elif key == "target":
            return self.targetVar.get()
        elif key == "model":
            return self.modelVar.get()
        elif key == "which.des":
            return self.whichDesVar.get()
        elif key == "key.size":
            return self.keySizeVar.get()
        return 0

    def getBooleanFromPanel(self, checkbox, key):
        return self.tdesVar.get()

    def getKeyValues(self):
        self.roundKeyString = self.roundKeyTextField.get().strip()
        if len(self.roundKeyString) > 0:
            try:
                self.roundKey = int(self.roundKeyString.replace(' ', ''), 16)
                self.set("round.key", self.roundKey)
            except ValueError:
                pass
        desKeyString = self.desKeyTextField.get().strip()
        if len(desKeyString) > 0:
            try:
                ...
                self.set("des.1.key", self.desKey)
            except ValueError:
                pass
        desKeyString = self.des2KeyTextField.get().strip()
        if len(desKeyString) > 0:
            try:
                hex_str = desKeyString.replace(' ', '')
                ba = bytes.fromhex(hex_str)
                self.des2Key = self.toLong(ba) if len(ba) <= 8 else self.toLong(ba, len(ba) - 8, 8)
                self.set("des.2.key", self.des2Key)
            except ValueError:
                pass
        desKeyString = self.des3KeyTextField.get().strip()
        if len(desKeyString) > 0:
            try:
                hex_str = desKeyString.replace(' ', '')
                ba = bytes.fromhex(hex_str)
                self.des3Key = self.toLong(ba) if len(ba) <= 8 else self.toLong(ba, len(ba) - 8, 8)
                self.set("des.3.key", self.des3Key)
            except ValueError:
                pass
    ...
    def keyReleased(self, event):
        ...
        if tdes and self.keySize == 0:
            if event.widget == self.des3KeyTextField:
                self.desKeyTextField.delete(0, tk.END)
                self.desKeyTextField.insert(0, self.des3KeyTextField.get())
            elif event.widget == self.desKeyTextField:
                self.des3KeyTextField.delete(0, tk.END)
                self.des3KeyTextField.insert(0, self.desKeyTextField.get())
        self.actionPerformed()

    def select(self, var1):
        attackObj = DesSboxInOutHDDefault()
    ...
    def reportKeyRetrieval(self, var1, var2):
        ...
        if var1 >= self.keys - 1:
            var3 = self.formatNumberString(format(self.newRoundKey, 'x').upper(), 2)
            if self.round != 0 and self.round != 3:
                var4 = self.deRotate(self.invPermuteText(self.roundKey, Des.PC2, 56, 'x'),
                                     1 if self.selectedRound == 1 else 16)
                var4 = self.invPermuteTextFromString(var4, Des.PC1, 64, '0')
                var5 = self.deRotate(self.invPermuteText(self.newRoundKey, Des.PC2, 56, 'x'),
                                     2 if self.selectedRound == 1 else 15)
                var5 = self.invPermuteTextFromString(var5, Des.PC1, 64, '0')
                print(f"DES 第 {1 if self.selectedRound == 1 else 16}轮子密钥: {var4}")
                print(f"DES 第 {2 if self.selectedRound == 1 else 15}轮子密钥: {var5}")
                var6 = list(var5)
                self.mismatches = 0
                for var7 in range(len(var4)):
                    if var4[var7] != 'x' and var5[var7] != 'x' and var4[var7] != var5[var7]:
                        print(f"第 {var7} 比特不匹配")
                        self.mismatches += 1
                    if var4[var7] != 'x':
                        var6[var7] = var4[var7]
                var3 = ''.join(var6)
                var13 = int(var3, 2)
                var3 = format(var13, 'x').upper()
                while len(var3) < 16:
                    var3 = '0' + var3
                var3 = self.formatNumberString(var3, 2)
                var8 = var13.to_bytes((var13.bit_length() + 7) // 8, 'big')
                if self.targetDes == 0:
                    self.set("des.1.key", self.toLong(var8) if len(var8) <= 8 else self.toLong(var8, len(var8) - 8, 8))
                    if self.keySize == 0:
                        self.set("des.3.key",
                                 self.toLong(var8) if len(var8) <= 8 else self.toLong(var8, len(var8) - 8, 8))
                elif self.targetDes == 1:
                    self.set("des.2.key", self.toLong(var8) if len(var8) <= 8 else self.toLong(var8, len(var8) - 8, 8))
                else:
                    self.set("des.3.key", self.toLong(var8) if len(var8) <= 8 else self.toLong(var8, len(var8) - 8, 8))
                    if self.keySize == 0:
                        self.set("des.1.key",
                                 self.toLong(var8) if len(var8) <= 8 else self.toLong(var8, len(var8) - 8, 8))
            else:
                self.set("round.key", self.newRoundKey)
                print(f"轮密钥: {var3}")
